            ...
            <div className="w-full flex flex-col mt-2">
                <div className="flex flex-wrap md:flex-nowrap items-end gap-3 w-full">
                  ...
                  <button 
                    ...
                    className="w-[46px] md:w-[130px] shrink-0 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-sm h-[46px] rounded-xl transition-all flex items-center justify-center gap-1.5 border border-gray-200 cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 bg-primary hover:bg-primary/90 text-white font-bold text-sm md:text-base h-[46px] rounded-xl transition-all flex items-center justify-center shadow-lg shadow-primary/20 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer min-w-[120px]"
                  >
                    {t("অর্ডার দিন", "Order Now")}
                  </button>

                  <a
                    href={`https://wa.me/8801738364268?text=${encodeURIComponent(
                      `আসসালামু আলাইকুম, আমি রকমারি পণ্য হাড়ি থেকে এই পণ্যটি হোয়াটসঅ্যাপে অর্ডার করতে চাই:\n\n` +
                      `📌 পণ্য: ${selectedProduct.name}\n` +
                      `💰 মূল্য: ৳${getProductPrice(selectedProduct, tempSelectedQty) * tempSelectedQty}\n` +
                      (tempSelectedColor ? `🎨 কালার: ${tempSelectedColor}\n` : '') +
                      (tempSelectedSize ? `📏 সাইজ: ${tempSelectedSize}\n` : '') +
                      `📦 পরিমাণ: ${tempSelectedQty}\n` +
                      `🔗 লিংক: ${window.location.origin}/p/${selectedProduct.slug || selectedProduct.id}`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full sm:w-auto px-4 bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold text-xs md:text-sm h-[46px] rounded-xl transition-all flex items-center justify-center gap-2 shadow-md shadow-[#25D366]/20 active:scale-95 cursor-pointer whitespace-nowrap"
                    title="হোয়াটসঅ্যাপে সরাসরি অর্ডার করুন (০১৭৩৮-৩৬৪২৬৮)"
                  >
