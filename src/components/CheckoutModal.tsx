              ...
              <div className="bg-white p-5 rounded-2xl border border-gray-100 space-y-4">
                ...
                <div className="pt-1 sticky bottom-4 z-50 shadow-2xl md:shadow-none md:static">
                  <button
                    ...
                    className="w-full bg-primary hover:bg-primary/95 text-white font-bold text-sm py-3 rounded-xl transition-all active:scale-95 flex items-center justify-center gap-1.5 disabled:opacity-50 font-sans cursor-pointer"
                  >
                    {isOrderProcessing ? (
                  ...
                  <a
                    href={`https://wa.me/8801738364268?text=${encodeURIComponent(
                      `আসসালামু আলাইকুম, আমি রকমারি পণ্য হাড়ি থেকে এই পণ্যগুলো হোয়াটসঅ্যাপে অর্ডার করতে চাই:\n\n` +
                      checkoutItems.map((item: any, idx: number) => `${idx + 1}. ${item.product.name} (পরিমাণ: ${item.quantity}) - ৳${(getProductPrice(item.product, item.quantity) || item.product.price) * item.quantity}`).join('\n') +
                      `\n\nমোট বিল: ৳${checkoutItems.reduce((acc: number, curr: any) => acc + (getProductPrice(curr.product, curr.quantity) || 0) * curr.quantity, 0)}\nদয়া করে আমার অর্ডারটি কনফার্ম করুন।`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full mt-2.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#128C7E] font-bold text-xs py-2.5 px-3 rounded-xl transition-all flex items-center justify-center gap-2 border border-[#25D366]/30 cursor-pointer"
                  ...
                  >
                    ...
                    <span>হোয়াটসঅ্যাপে অর্ডার করুন (০১৭৩৮-৩৬৪২৬৮)</span>
                  </a>
                </div>
